    ...
    def __init__(self):
        ...
        def build_base_network():
            with tf.device('/device:GPU:0'):
                ...
                input = tf.concat([flattened_vec,y_bid,y_mid], 1)

                dense1 = tf.layers.dense(inputs=flattened_vec, units=2659,
                                         kernel_initializer=tf.keras.initializers.he_uniform())
    ...
    def train(self,restore=False):
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,log_device_placement=False)) as sess:
            try:
                ...
                if restore:
                    saver.restore(sess, tf.train.latest_checkpoint('./saves_s/'))
                else:
                    ...
                    if counter%1000 == 0:
                        acc_s= sess.run([self.accuracy_s],
                                       feed_dict={self.x_placeholder:val_x, self.y_b_placeholder: val_b, self.y_m_placeholder: val_m,
                                                  self.y_s_placeholder: val_s, self.training:False})
                        print('s :', acc_s[0])
                        ...
                        if acc_s[0] > 0.59:
                            ...
                            save_path = saver.save(sess, './saves_s/model_%d.ckpt'%check_step)
