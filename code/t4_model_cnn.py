    ...
    def __init__(self):
        ...
        def build_base_network():
            with tf.device('/device:GPU:0'):
                ...
                y_did = tf.placeholder_with_default(y_did, shape=(None, 405), name='ydid_placeholder')
                ...
                dense_d = tf.layers.dense(inputs=bn_d1, units=405,
                                          kernel_initializer=tf.keras.initializers.he_uniform())

                output_d = tf.nn.softmax(dense_d)

                prediction_d = tf.argmax(output_d, axis=1)

                equality_d = tf.equal(prediction_d, tf.argmax(y_did, axis=1))

                accuracy_d = tf.reduce_mean(tf.cast(equality_d, tf.float32))

                loss_d = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_did, logits=dense_d, name='did_loss'))

                update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)


                with tf.control_dependencies(update_ops):
                    step = tf.train.AdamOptimizer(learning_rate=0.0001).minimize(loss_d)
            self.loss_d = loss_d
            ...
            self.accuracy_d = accuracy_d
            self.prediction_d = prediction_d
            self.step = step
            self.iterator_val = iterator_val
            self.iterator_infer = iterator_infer

            tf.summary.scalar("loss", self.loss_d)
            tf.summary.scalar('train accuacy', self.accuracy_d)
    ...
    def train(self,restore=False):
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,log_device_placement=False)) as sess:
            try:
                ...
                if restore:
                    saver.restore(sess, tf.train.latest_checkpoint('./saves_t4'))
                else:
                ...
                print("build cnn model for did prediction")
                while True:
                    counter += 1
                    ...
                    if counter%1000 == 0:
                        acc_d= sess.run([self.accuracy_d],
                        ...
                        accuracy_summary = tf.Summary(value=[tf.Summary.Value(tag='Val_Accuragy',
                                                                              simple_value=acc_d[0])])
                        train_writer.add_summary(accuracy_summary, counter)

                        if acc_d[0] > 0.929:
                            check_step += 1
                            print("saving acc = %f"%acc_d[0],"Saving model ...")
                            save_path = saver.save(sess, './saves_t4/model_%d.ckpt'%check_step)
    ...
    def infer(self, batch_size):
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,log_device_placement=False)) as sess:
            ...
            for i in range((507783//batch_size) + 1):
                ...
                pred = sess.run([self.prediction_d], feed_dict={self.x_placeholder: val_x, self.training: False})

                print('Tear D, Continuing... {}'.format(i*batch_size))
